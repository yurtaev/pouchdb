...
function WebSqlPouch(opts, callback) {
  ...
  api._nativeQuery = function (name, fun, opts, callback) {
    var keySig = 'native-' + utils.MD5(name) + '-key';
    var valueSig = 'native-' + utils.MD5(name) + '-value';
    db.transaction(function (tx) {
      var sql = 'SELECT COUNT(*) FROM ' + BY_SEQ_STORE + ' WHERE ' + keySig
        ' IS NULL';
      tx.executeSql(sql, [], function (tx, result) {

      }, function () {
        // error; it doesn't exist yet
        var sql = 'ALTER TABLE ' + BY_SEQ_STORE + ' ADD COLUMN ' + keySig;
        tx.executeSql(sql, [], function () {
          var sql = 'ALTER TABLE ' + BY_SEQ_STORE + ' ADD COLUMN ' + valueSig;
          tx.executeSql(sql, [], function () {
