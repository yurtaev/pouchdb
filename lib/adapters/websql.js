...
function WebSqlPouch(opts, callback) {
  ...
  api._info = function (callback) {
    countDocs(false, function (docCount) {
      db.transaction(function (tx) {
  ...
  function applyNextDocCount() {
    var docCount = docCounts[name];
    if (!docCount || docCount.running || !docCount.queue.length) {
      return;
    }

    docCount.running = true;
    var item = docCount.queue.shift();

    function complete() {
      if (item.callback) {
        item.callback(docCount.docCount);
      }
      docCount.running = false;
      applyNextDocCount();
    }

    if (!item.refresh && docCount.docCount !== -1 && !docCount.running &&
        !(docCount.queue.length && docCount.queue.filter(function (item) {
          return item.refresh;
        }).length)) {
      return complete();
    }
    ...
    // count() does a full table scan, so we cache it
    db.transaction(function (tx) {
      var sql = 'SELECT COUNT(' + DOC_STORE + '.id) AS \'num\' FROM ' +
        ...
        DOC_STORE + '.local = 0';
      tx.executeSql(sql, [], function (tx, result) {
        docCount.docCount = result.rows.item(0).num;
        complete();
      });
    }, function () {
      docCount.docCount = 0;
  ...
  function countDocs(update, callback) {
    var docCount = docCounts[name];
    docCount.queue.push({callback : callback, update : update});
    applyNextDocCount();
  }
  ...
  api._allDocs = function (opts, callback) {
    ...
    countDocs(false, function (count) {
      db.transaction(function (tx) {
