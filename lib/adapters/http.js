  ...
  api._changes = function (opts) {
    ...
    var fetch = function (since, callback) {
      ...
      xhr = ajax(xhrOpts, function (err, res) {
        if (err) {
          if (err.name === 'bad_request' && err.message ===
              "Malformed sequence supplied in 'since' parameter." &&
              opts.since === 'now') {
            // cloudant workaround until ?since=now is supported
            // (see https://github.com/cloudant/fabric/pull/152)
            xhr = ajax({
              headers: host.headers,
              method: 'GET',
              url: genDBUrl(host, '_changes?limit=0'),
              timeout: opts.timeout
            }, function (err, res) {
              if (err) {
                return callback(err);
              }
              xhrOpts.since = res.last_seq;
              xhr = ajax(xhrOpts, callback);
            });
            return;
          } else if (err.name === 'bad_request' && err.message ===
              "filter parameter must be of the form `designname/filtername`" &&
              opts.filter === '_view' && opts.view) {
            // cloudant workaround for how they define filter views
            xhrOpts.url = genDBUrl(host, '_changes' + paramStr.replace(
              /filter=.*?($|&)/, 'filter=' + opts.view + '$1'));
            xhr = ajax(xhrOpts, callback);
          } else {
            callback(err);
          }
