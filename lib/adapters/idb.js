...
function init(api, opts, callback) {
  ...
  function onReceiveIDBInfo(err, info) {
    if (info) {
      idb = info.idb;
      blobSupport = info.blobSupport;
      instanceId = info.instanceId;
      idStored = info.idStored;
    }
  ...
  if (cached) {
    onReceiveIDBInfo(null, cached);
    return;
  }

  openIDB(name, function (err, info) {
    if (info) {
      cachedDBs[name] = info;
    }
    onReceiveIDBInfo(err, info);
  });
}
...
function destroy(name, opts, callback) {
  ...
  if (openReqList[name] && openReqList[name].result) {
    openReqList[name].result.close();
  }
