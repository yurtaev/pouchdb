...
function init(api, opts, callback) {
  ...
  function countDocs(txn, callback) {
    if (docCount !== -1) {
  ...
  api._allDocsContext = function (errorCallback, callback) {
    var txn = idb.transaction([DOC_STORE, BY_SEQ_STORE], 'readonly');
    txn.onerror = idbError(errorCallback);
    // optimization - we know if someone's requesting this, then they
    // plan to do an allDocs, so we'll prime the docCount in advance
    countDocs(txn, function () {
      callback(txn);
    });
  };
  ...
  api._allDocs = function idb_allDocs(opts, callback) {
    ...
    var txn = opts.ctx;
    if (!txn) {
      txn = opts.ctx = idb.transaction([DOC_STORE, BY_SEQ_STORE], 'readonly');
      txn.onerror = idbError(callback);
    }

    // first count the total_rows
    countDocs(txn, function (err, totalRows) {
      if (err) {
        return callback(err);
      }
      if (opts.limit === 0) {
  ...
  api._info = function idb_info(callback) {
    ...
    countDocs(txn, function (err, count) {
      if (err) {
        return callback(err);
      }
      if (idb === null) {
      ...
      txn.objectStore(META_STORE).get(META_STORE).onsuccess = function (e) {
        var updateSeq = e.target.result && e.target.result.updateSeq || 0;
        callback(null, {
          db_name: name,
          doc_count: count,
