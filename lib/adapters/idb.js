...
function init(api, opts, callback) {
  ...
  api._bulkDocs = function idb_bulkDocs(req, opts, callback) {
      ...
      function finish() {
        ...
        function afterPut(e, newEdit) {
          ...
          metaDataReq.onsuccess = function () {
            ...
            if (!fetchedDocs[docInfo.metadata.id] && !newEdit) {
              // can only increment the update_seq if this is truly a new
    ...
    function updateDoc(oldDoc, docInfo) {
      ...
      if (oldDoc.rev_tree.length < merged.tree.length) {
        // more leaves than before at the root,
        // i.e. we've introduced a conflicting "1-" rev
        updateSeq--; // discount
      }
