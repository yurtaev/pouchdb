...
function LevelPouch(opts, callback) {
  ...
  api._bulkDocs = writeLock(function (req, opts, callback) {
    ...
    function writeDoc(doc, index, isUpdate, callback2) {
      ...
      function autoCompactAndGetBatch() {
        if (!isUpdate || !api.auto_compaction) {
          return []; // nothing to do
        }
        var revsToDelete = utils.compactTree(doc.metadata);
        var revMap = doc.metadata.rev_map; // map from revs to seqs
        return revsToDelete.map(function (rev) {
          return {
            key: formatSeq(revMap[rev]),
            type: 'del',
      ...
      function finish() {
        ...
        var batch = autoCompactAndGetBatch();
