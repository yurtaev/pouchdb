    ...
    if (opts.continuous) {
      changeListener = function (change) {
        ...
        if (filter(change) && !opts.done) {
          var changeRev = change.changes[0].rev;
          if (!alreadyCalledOnChange[changeRev]) {
            called++;
            opts.onChange(change);
          } else {
            // avoid memory leak here
            delete alreadyCalledOnChange[changeRev];
          }
        }
      };
    ...
    var throughStream = through(function (data, _, next) {
      ...
      stores.docStore.get(data.value._id, function (err, metadata) {
        ...
        if (parseSeq(data.key) === metadata.rev_map[change.doc._rev] &&
          filter(change)) {
          alreadyCalledOnChange[change.changes[0].rev] = true;
          called++;
