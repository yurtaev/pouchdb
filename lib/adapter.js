...
function allDocsKeysQuery(api, opts, callback) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    function onGetContext(context) {

      var gotError;
      var numRecv = 0;

      function checkDone() {
        if (++numRecv === keys.length) {
          if (gotError) {
            reject(gotError);
          } else {
            resolve(results);
      ...
      function queryForKey(i) {
        ...
        api._allDocs(subOpts, function (err, res) {
          if (err) {
            ...
            results.total_rows = res.total_rows;
            results.rows[i] = res.rows[0] || {key: key, error: 'not_found'};
          }
          checkDone();
        });
      }
    ...
    if (typeof api._allDocsContext === 'function') {
      api._allDocsContext(callback, onGetContext);
    } else {
      onGetContext();
    }
  });
